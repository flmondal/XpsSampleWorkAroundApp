...
System::Void Decryptor::initIpcState()
{
	m_key = nullptr;
...
//bool Decryptor::AccessCheck(String ^permissionName)
//{
//	bool                    granted;
//	HRESULT                 hr;
//	BOOL                    allowed;
...
MemoryStream ^Decryptor::Decrypt(array<Byte> ^encryptedBytes)
{
...
MemoryStream ^Decryptor::Decrypt(Stream ^encryptedStream)
{
	MemoryStream                ^result;
...
////
//
//System::Void Decryptor::getPolicyInformation()
//{
...
////
//
//System::Void Decryptor::readPreamble(Stream ^encryptedStream)
//{
//	array<Byte>     ^preamble;
...
//
//
System::Void Decryptor::readLicense(Stream ^encryptedStream)
{
...
MemoryStream ^Decryptor::readBody(Stream ^encryptedStream)
{
	...
	try
	{
		...
		//readBuffer = gcnew array<Byte>(cbMyBlockSize);
		pbReadBuffer = &readBuffer[0];

		cbOutputBuffer = *pcbBlockSize;
		//cbOutputBuffer = cbMyBlockSize;
		...
		while (cbReadRemaining > cbMyBlockSize)
		{
			cbRead = 0;
			encryptedStream->Read(readBuffer, 0, cbMyBlockSize);
			hr = IpcDecrypt(m_key,  //M_KEY ???????m_key
				cBlock,
				false,
				pbReadBuffer,
				cbMyBlockSize,
	...
	finally
	{
		readBuffer = nullptr;
		writeBuffer = nullptr;
		IpcFreeMemory((LPVOID)cbMyBlockSize);
	}
